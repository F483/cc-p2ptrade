...
EWalletController.prototype.selectInputs = function(colorValue, cb){
  ...
  if(colorValue.isUncolored()){
    ...
    optx.selectCoins(colorValue, feeEstimator, function(error, coins, total){
    ...
    optx.selectCoins(colorValue, null, function(error, coins, total){
      if(error){ return cb(error) }
      var change = total.minus(colorValue)
      cb(error, coins, change)
    })
  }
}
...
EWalletController.prototype.makeReplyTx = function(etxSpec, our, their, cb){
  ...
  opTxSpec.prepareInputs(etxSpec, function(error){
    if(error){ return cb(error) }
