...
MyEProposal.prototype.processReply = function(replyEP, cb){
  var self = this
  var rawTx = RawTx.fromHex(replyEP.etx_data)
  var wallet = self.ewctrl.wallet
  var seedHex = self.ewctrl.getSeedHex()
  rawTx.sign(wallet, seedHex, function(error){
    if(error){ return cb(error) }

    var am = wallet.getAddressManager()
    rawTx.getOutputAddresses(wallet).forEach(function(address){
      var inwallet = !!am.getPrivKeyByAddress(address, seedHex)
    })
